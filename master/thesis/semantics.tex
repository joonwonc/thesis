...
inlining for a module is problematic. TODO: explain more.
\begin{figure}[h]
...
In order to avoid such cases, the method-inlining operator checks
whether the method which will be inlined has a \emph{self-recursive}
method call or not. As described in \refdef{def-inlinedmm},
(\isrec{dm}) ensures that $dm$ is not recursive. We have proved the
self-recursive call check for each method in a module during the whole
inlining implies that there is no call cycles in the original
module. (See \refsect{TODO} for details)

We naturally extend the method-inlining operator to a methods-inlining
operator $(\inlinedmssymb)$.
\begin{definition}
  \label{def-inlinedms}
  \mbox{}
  \begin{center}
...
\paragraph{Filtering internal methods}

Ipsum

\paragraph{Inlining semantics}
