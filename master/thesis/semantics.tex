...
consist of a write to a register, a method call, an assert, and a
return.
...
including the implication proof in \refchap{chap:implication}.
...
cycle. These cases are similar to $\epsilon$-steps in
CompCert~\cite{leroy-compcert-cacm}. However, an empty step by a rule
and one by a method should be separately defined in the \Substep{}
semantics, in order to give explicit information when defining the
refinement relation.
