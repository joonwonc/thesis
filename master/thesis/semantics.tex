...
In this chapter, we present a formal definition of inlining. The
inlining operation is defined on the Kami language and the modular
semantics~\cite{murali-thesis}. Finally, we present a new semantic
approach to make up for the weakness of the modular semantics.
...
names. An assert action takes an expression $e$ to be checked to
progress the continuation. Lastly, a return action takes $e$ as a
return value. Note that all action components except a return action
also takes an action $a$ as a continuation.
...
different version of it, in which the current \Kami{} framework
uses. The motivation is kept unchanged: semantic rules are defined
with respect to module definitions, which include the one for
combining modules. The semantics defined in this section will be used
throughout the thesis, including the implication proof in
...
Semantics for variable is not defined, since it should be substituted
to the value by $\beta$-reduction on higher semantics. Technically, in
the \Kami{} framework, syntax is defined by PHOAS~\cite{adam-icfp},
thus the variable term is abstracted into a syntax constructor
$\textsf{Var}$. In this case, we rather can define
...
A label is firstly used on this semantics, in order to represent
...
indicating methods executed in a cycle. Similarly, $cs$ denotes
\emph{called methods}, indicating methods which are called in a cycle.
...
in a cycle. These cases are similar to $\epsilon$-steps in the C
language behaviors defined in CompCert~\cite{leroy-compcert-cacm}.
However, an empty step by a rule and one by a method should be
...
In this section, we present one of well-formedness conditions of
...
synthesized circuit would make structural hazards. If not satisfying
the latter, then the call cycles will form a combinational cycle, in
which the behavior of it is nonpredictable. The common reason for
checking well-formedness conditions is that behaviors (semantics)
cannot be defined without them. Thus, these conditions are also
employed to verify properties of hardware designs.

Whereas, some conditions cannot be statically checked, thus should be
defined in part of semantics. For instance, when combining two
...
In this thesis, we define a well-formedness condition ensuring that
there are no double writes or double calls. For a given module $m$,
$(\wfdouble{m})$ ensure such a property. The condition is used to
prove the implication between semantics, which is one of main topics
of the thesis (See \refchap{chap:implication} for details).
