...
Later they tried to verify another procotol with a more-developed TLC, but the verification was still limited to a single cache line, two data values, and three processors, due to the state-space explosion to twelve million reachable states.
...
\revised{
There was an approach trying to prove both correctness and liveness of adaptive cache-coherence protocols using the PVS theorem prover~\cite{Shen:1999,Stoy:Shen:2001,thesis:Shen:2000}.
...
In order to verify the protocol more easily, they suggested to design and prove protocols by two stages: one for imperative rules, which are only about correctness, and the other for directive rules that affect both correctness and liveness.
