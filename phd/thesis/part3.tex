...
\begin{figure}[t]
  ...
  \tikzset{
    hemobj/.pic = {
      ...
      \draw [arrow, line width=1.2pt] (hrules) to [out=90,in=-90] node[right, pos=0.75] {\footnotesize\tt{outputs}} (houtput);
    },
    kamimod/.pic = {
      ...
      \node [comp, inner sep=2pt, text width=3.4cm, minimum height=1.2cm] at (0.5, -1.2) {{\small\tt BRAM-based caches (defined in Kami)}};

      \node (input1) at (-5.2, 1.4) {};
      \node (input2) at (-5.2, 0.2) {};
      \node (output) at (5.4, 0.8) {};
      ...
      \draw [arrow, line width=0.8pt] (in1) to[out=0,in=90,distance=0.2cm] (mshrs);
      \draw [arrow, line width=0.8pt] (in2) to[out=0,in=90,distance=0.2cm] (mshrs);
      ...
      \draw [arrow, line width=1.2pt] (ex) to node[above=2pt, pos=0.85] {\footnotesize\tt{outputs}} (output);
    }
  }
  \begin{tikzpicture}
    \pic at (0, 2.6) {hemobj};
  ...
  \end{tikzpicture}
  \caption{Compilation of a cache controller}
...
\autoref{fig-compilation-pipeline} depicts a source \hemiola{} cache object and the corresponding target Kami module, generated by the protocol compiler.
Each \hemiola{} rule in the source cache object takes input messages from various channels, accesses values -- a line status, a line value, and locks --, and generates output messages atomically at a step.
