...
\begin{figure}[t]
  ...
  \begin{tabular}{cc}
    \begin{tikzpicture}
      ...
      % Curves
      \draw [->,color=myred] (-2.95, -2.05) to[out=90,in=-110] node[left=3pt] {\rdcircflsm{1}{rquu}} (-2.35, -1.05);
      \draw [->,color=myred] (-2.25, -0.5) to[out=70,in=110,distance=2cm] node[above=2pt] {\rdcircflsm{3}{rqud}} (1.7, -0.2);
      \draw [->,color=myred] (2.2, -0.7) to[out=-40,in=-40,distance=1.2cm] node[below right=-3pt] {\rdcircflsm{4}{immu}} (2.0, -1.1);
...
We use an arrow ({\color{myred} $\to$}) and a label (\rdcircfl{1}{\textrm{rquu}}) to denote a state transition by a rule defined with a certain rule template, \eg{} \rdcircfl{1}{rquu} is a rule in $C_1$ using the {\bf rquu} rule template, taking an input message \idmsf{1}{rqWr} and outputting \idmsf{3}{rqM}.

$\rdcircf{1}$ (or $\blcircf{2}$) uses the {\bf rquu} rule template.
It takes \msgsfsm{rqWr} from the external-input channel of $C_1$ (or $C_2$) and makes a request \msgsfsm{rqM} to the parent $P$ to obtain the M status.
...
$\rdcircf{3}$ (or $\blcircf{6}$) uses the {\bf rqud} rule template.
It takes \msgsfsm{rqM} from the upward-request channel of $C_1$ (or $C_2$) and makes an invalidation request \msgsfsm{rqI} to the other child $C_2$ (or $C_1$).
...
Indeed, after $\rdcircf{3}$ is executed, \blrecmsgsm{f}{rqM} has been stalled until the downlock is released by $\rdcircf{5}$.

$\rdcircf{4}$ (or $\blcircf{8}$) uses the {\bf immu} rule template.
It takes \msgsfsm{rqI} from the downward channel to $C_2$ (or $C_1$) and responds to the parent with \msgsfsm{rsI}.
Note that {\bf immu} does not require the uplock be free.
Indeed, at the moment $\rdcircf{4}$ is executed, $C_2$ has been uplocked by \blcircfl{2}{rquu}.

$\rdcircf{5}$ uses the {\bf rsud} rule template.
It takes \msgsfsm{rsI} from the upward-response channel from $C_2$ and responds back to $C_1$ with \msgsfsm{rsM}.
{\bf rsud} by definition releases the downlock at this moment so $P$ can take another upward request (\eg{} \blrecmsgsm{f}{rqM}).

Lastly, $\rdcircf{7}$ uses the {\bf rsdd} rule template.
It takes \msgsfsm{rsM} from the downward channel to $C_1$ and responds back with \msgsfsm{rsWr} to the external world.
...
The use of noninterference in Hoare logic has become more explicit later by several variants like concurrent separation logic (CSL)~\cite{OHearn:2004} and rely-guarantee reasoning~\cite{Jones:1983}.
...
This rule requires to define a resource invariant $RI_r$, stating desired properties when a process holds $r$.
Mutual exclusion by a shared lock is one of typical ways to ensure noninterference.
The uplock and downlock used in \hemiola{} function similarly.
For example, acquiring a downlock means entering a ``subtree'' section (although not entirely critical) whose root is the downlocked object; a transaction holding the downlock iterates the subtree, makes state transitions, and releases it when getting out of the subtree.
...
There is still certain concurrency in executing a single transaction; \eg{} after a parent cache makes invalidation requests to children, each child (subtree) handles its request concurrently.
...
In the sense of dealing with trivial concurrency, we would like to note that using the predicate messages for input messages to prove the one for an output message is quite similar to the Parallel Composition Rule in CSL.
