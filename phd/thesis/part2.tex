...
We will provide more detailed discussions on the use of commuting reductions in \autoref{sec-related-work-i}.
...
We first present our intuition of how commuting reductions are used in the serializability proof; in order for it, we need to formalize the reduction first.
...
\begin{theorem}
  ...
  In any legal history in the system $S$, the first external-input (the last external-output) label can be pushed to the first (the end) of the history.
  \begin{displaymath}
...
\end{itemize}
Note that pushing the external input (output) labels to the leftmost (rightmost), is possible since the semantics of the protocol transition system is based on infinite-sized buffers, allowing the system to accept all the external input messages as early as possible and to postpone releasing all the external output messages as late as possible.
