...
This object commutativity is naturally extended to the one for atomic histories:
\begin{definition}\label{def-obj-comm}
  For given atomic histories $h_1$ and $h_2$, we say $h_1$ and $h_2$ are object-commutative, denoted as $(\objseph{h_1}{h_2})$, if the following conditions are satisfied:
  \begin{displaymath}
    \renewcommand{\arraystretch}{1.2}
    \begin{array}{rcl}
...
The intuition here is that $h_1$ and $h_2$ are commutative in terms of object state transitions when each label -- representing an object state transition -- in $h_1$ and $h_2$ are commutative either by different objects or by the commutativity conditions within the same object.
...
\begin{definition}\label{def-msg-comm}
  For given atomic histories $h_1$ and $h_2$, we say $h_1$ and $h_2$ are message-commutative, denoted as $(\msgseph{h_1}{h_2})$, if the following conditions are satisfied:
  \begin{displaymath}
    \renewcommand{\arraystretch}{1.2}
    \begin{array}{rl}
...
Using the new, relaxed object and message commutativity, we present the following reduction theorem:
\begin{theorem}[Reduction by separation]
  \label{thm-red-by-sep}
  \begin{displaymath}
    \forall h_1, h_2.\; \atomicShort{h_1} \;\wedge\; \atomicShort{h_2} \;\wedge\;
    \objseph{h_1}{h_2} \;\wedge\; \msgseph{h_1}{h_2} \to \commhst{h_1}{h_2}.
  \end{displaymath}
\end{theorem}
\begin{proof}
  ...
  Then we obtain the object commutativity $(\objsep{r_1}{r_2})$ from $(\objseph{h_1}{h_2})$.
...
%% The basic merging strategy can be summarized as follows:
%% \begin{enumerate}
%% \item For each atomic history $h \in \listof{h}$, show that the following property holds:
%%   \begin{displaymath}
%%     \begin{array}{l}
%%       \mathsf{Separated}_2\ h_1\ h_2\ h\ \triangleq
%%       \quad\quad (\aobjs{h} \subseteq (\aobjs{h_1} - \aobjs{h_2}) \;\wedge\; \achns{h} \subseteq (\achns{h_1} - \achns{h_2})) \;\vee\\
%%       \quad\quad (\aobjs{h} \subseteq (\aobjs{h_2} - \aobjs{h_1}) \;\wedge\; \achns{h} \subseteq (\achns{h_2} - \achns{h_1})),
%%     \end{array}
%%   \end{displaymath}
%%   which implies $h$ is disjoint to either $h_1$ or $h_2$.
%% \item
%% \end{enumerate}
