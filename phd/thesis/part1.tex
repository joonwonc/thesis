...
In practical cache-coherence protocols, however, only specific message-passing patterns are used.
Recalling our interleaving example in \autoref{sec-nutshell}, we see that the three objects ($P$, $C_1$, and $C_2$) form a tree shape and communicate through the designated three channels between the parent and a child.
One of the main intuitions of the \hemiola{} framework is that safe interleavings are ensured just by restricting the communication patterns among objects (with proper locking mechanisms).
We will explore how such a restriction is enforced by defining a domain-specific language (DSL) in \autoref{sec-hemiola-dsl} and how the DSL implies safe interleavings in \autoref{sec-sz-guarantee-hemiola}.
} %% end of \revised
...
It nondeterministically chooses an object and a rule in the object, checks that the precondition holds ($r.p\, (o_1, \msgIns{im})$), and applies the transition to update the state of the system ($r.t\, (o_1, \msgIns{im}) = (o_2, \msgOuts{im})$).
...
In contrast to the input messages, each output message should be enqueued to either an internal channel or an external-response one ($\midxOuts{im} \subseteq \hsysInA{S} \cup \hsysRsA{S}$).
Lastly, the channels of the input and output messages should be disjoint from each other ($\disj{\midxIns{im}}{\midxOuts{im}}$).
...
\section{Cache-Coherence Protocols as Distributed Protocols}

We already explored in \autoref{sec-nutshell} with the interleaving example that the cache objects in a cache-coherent memory system handle multiple requests from the processor cores concurrently, in a distributed way.
...
\begin{figure}[t]
  ...
  \begin{tabular}{c}
    \begin{tikzpicture}
      ...
      % Curves
      \draw [->,color=myred] (-2.3, -2.0) to[out=90,in=-90] node[left=3pt,color=black] {$r_1$} (-1.8, -1.0);
      \draw [->,color=myred] (-1.7, -0.4) to[out=70,in=110,distance=1.8cm] node[left=20pt,color=black] {$r_2$} (1.4, -0.4);
      \draw [->,color=myred] (1.6, -0.9) to[out=-50,in=-50,distance=1.9cm] node[below,color=black] {$r_3$} (0.5, -1.3);
      ...
      \node at (1.9, 0.3) {$r_4$};
      \draw [->,color=myred] (-0.6, -1.1) to[out=225,in=90] node[below right,color=black] {$r_5$} (-1.1, -2.0);
    \end{tikzpicture}\\
    \hline
    \begin{math}
      \begin{array}{rl}
        h \triangleq & [\lblInt{\idxOf{C_1}}{\idxOf{r_1}}{\listsingle{(1, {\sf rqWr})}}{\listsingle{(3, {\sf rqM})}};\\
          ...
          & \lblInt{\idxOf{P}}{\idxOf{r_4}}{\listsingle{(7, {\sf rsI})}}{\listsingle{(5, {\sf rsM})}};\\
          & \lblInt{\idxOf{C_1}}{\idxOf{r_5}}{\listsingle{(5, {\sf rsM})}}{\listsingle{(2, {\sf rsWr})}}]\\
      \end{array}
    \end{math}
  \end{tabular}
...
\revised{
\autoref{fig-ex-atomic-history} presents an example of an atomic history from the interleaving example presented in \autoref{sec-nutshell}.
$h$ is generated by executions of five rules, $r_1 \in \hobjrules{C_1}$, $r_2 \in \hobjrules{P}$, $r_3 \in \hobjrules{C_2}$, $r_4 \in \hobjrules{P}$, and $r_5 \in \hobjrules{C_1}$.
$r_1$ takes an input message $(1, \textsf{rqWr})$ (from the channel with index $1$) as an initial message of the history.
$r_2$ takes $(3, \textsf{rqM})$, the output message from $r_1$.
$r_3$ takes $(8, \textsf{rqI})$, the output message from $r_2$.
$r_4$ takes $(7, \textsf{rsI})$, the output message from $r_3$.
Finally, $r_5$ takes $(5, \textsf{rsM})$, the output message from $r_4$.
Summing up all the rule executions, by the definition of an atomic history we get the predicate $\atomic{\listsingle{(1,\textsf{rqWr})}}{h}{\listsingle{(2,\textsf{rsWr})}}$.
...
For example, taking the first three rule executions (by $r_1$, $r_2$, and $r_3$) will generate the following history:
\begin{displaymath}
  \renewcommand{\arraystretch}{1.0}
  \begin{array}{rl}
    h_p \triangleq & [\lblInt{\idxOf{C_1}}{\idxOf{r_1}}{\listsingle{(1, {\sf rqWr})}}{\listsingle{(3, {\sf rqM})}};\\
      & \lblInt{\idxOf{P}}{\idxOf{r_2}}{\listsingle{(3, {\sf rqM})}}{\listsingle{(8, {\sf rqI})}};\\
      & \lblInt{\idxOf{C_2}}{\idxOf{r_3}}{\listsingle{(8, {\sf rqI})}}{\listsingle{(7, {\sf rsI})}}].\\
  \end{array}
\end{displaymath}
$h_p$ is still an atomic history satisfying $\atomic{\listsingle{(1,\textsf{rqWr})}}{h_p}{\listsingle{(7,\textsf{rsI})}}$ and is not completed in the sense that the live message ($\textsf{rsI}$) is not a response sent on an external channel.
} %% end of \revised
...
\revised{
\autoref{fig-sz-ex} presents an example of a serializable history, brought from the example in \autoref{sec-nutshell}, where two histories (in {\color{myred} red} and {\color{myblue} blue}) are interleaved to form a history $h$.
A corresponding sequential history $h_{\textrm{seq}}$ is also presented, formed by concatenating two atomic histories in {\color{myred} red} and {\color{myblue} blue}.
Following the rule executions in $h_{\textrm{seq}}$, we see that it is indeed a legal history and reaches the same state as the original history $h$ does.
} %% end of \revised

\revised{
It is quite natural to think of sequential histories only with atomic histories, but why do we have to separate external input/output labels from atomic histories?
} %% end of \revised
...
\begin{definition}[Predicate Messages]
  A \temph{predicate message}, denoted as \spredmsg{S}{im}{P}, is an invariant over a system $S$ that additionally requires $im$, a pair of a channel and a message, to be in the system:
  \begin{displaymath}
...
We will use an even-shorter version \predmsg{m}{\mathcal{I}} when the channel that contains the message is not ambiguous.
The same notation \predmsg{\msgid{}}{\mathcal{I}} will be used most commonly to define a predicate message that holds for every message with a given ID (\msgid{}) with the clear channel.
