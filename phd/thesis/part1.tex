...
We already explored in \autoref{sec-nutshell} with a concrete example that the cache objects in a cache-coherent memory system handles multiple requests from the processor cores concurrently, in a distributed way.
We also learned that transactions to handle such requests are interleaved, so ensuring safe interleaving is crucial in designing and proving a cache-coherence protocol.

A very well-known approach to ensuring safe interleavings is to prove \emph{noninterference}, which claims that no other state transitions affect state relevant to an ongoing transaction spuriously.
Not constrained to the cache-coherence protocols, noninterference has been developed and employed in some computer systems including concurrent software and distributed systems.
...
Serializability has been actively developed and used in database systems and software distribued systems.
To our knowledge, the notion of serializability has not been used to verify cache-coherence protocols.
