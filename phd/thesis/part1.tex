...
We will explore how such restriction is enforced by defining a domain-specific language (in \autoref{sec-hemiola-dsl}) and how it implies safe interleavings (in \autoref{sec-sz-guarantee-hemiola}).
} %% end of \revised
...
Following the rule executions in $h_{\textrm{seq}}$, we see that it is indeed a legal history and reaches the same state as the one by the original history $h$.
} %% end of \revised

\revised{
It is quite natural to think of sequential histories only with atomic histories, but why do we have to separate external inputs/outputs labels from atomic histories?
} %% end of \revised
