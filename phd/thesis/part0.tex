...
\revised{Throughout the dissertation, we will use this example to convey various intuitions in formal definitions and proofs, referred to as the ``interleaved history.''}

\section{Overview: the \hemiola{} Framework and Its Formal Guarantee}
\label{sec-overview}

In the last section, we confirmed that proper locking (by transient states) and topology are crucial for designing a correct cache-coherence protocol.
...
In this section, we provide the brief overview of the \hemiola{} framework by redefining the example protocol shown in \autoref{sec-nutshell} using rule templates.
%% We will use transient states we discussed in \autoref{sec-nutshell} directly in conventional descriptions.
We will also provide high-level intuitions about the formal meaning of safe interleavings and how the rule templates ensure this property.
...
\autoref{fig-overview-1} presents a conventional rule description (in pseudocode) used to request the M status to its parent on the left side of the figure.
This rule matches the rules $\rdcircf{1}$ and $\blcircf{2}$ in the interleaving example shown in \autoref{fig-motive-2}.
This figure also presents a corresponding rule in \hemiola{} using the proper rule template.
There are largely three benefits of using the \hemiola{} rule templates in this case; we explain them by matching the number annotations in the figure.
\begin{enumerate}[(1)]
\item Input/output patterns: in conventional description, it is required to designate a FIFO module to dequeue an input message and to check the purpose of the message by looking at its message ID.
  In \hemiola{}, each rule template is restricted in terms of input/output patterns; for instance, the \slstinline{rquu} rule template (where \slstinline{rquu} stands for request-up-up) always takes an input message from one of its children (\slstinline{:from} the \slstinline{ext}ernal world in this case) and generates an output message to the parent of \slstinline{:me C1}.
...
\item In-transition checks: conventional description uses a miss status holding register (\ccomph{\slstinline{mshr}}) to record whether a line is \slstinline{in_transition} or not.
  ...
  Hence, a user does not need to consider any in-transition checks or transient states, which is one of the major headaches in designing a protocol.
\item Use of transient states: again, in conventional desription, a user should manually update \ccomph{\slstinline{mshr}} to set a proper transient state and find a FIFO to enqueue an output message.
...
\autoref{fig-overview-2} shows another request rule pattern used in the parent object to make an invalidation request to the other sharer.
...
We have the same three benefits in this rule template: to use correct input/output channels, no need of in-transition checks, and no need to consider transient states.
...
The next pattern, shown in \autoref{fig-overview-3}, is used to respond to a request from a parent immediately.
(\slstinline{immu} stands for immediate-up.)
The rules $\rdcircf{4}$ and $\blcircf{8}$ in the interleaving example uses this pattern to handle an invalidation request.
...
In conventional description, it is required to change the transient state properly (from \slstinline{msiSM} to \slstinline{msiIM} in this example); in \hemiola{}, as seen in the figure, it is not required to manage such transient states.
%% The locking mechanism using an uplock and a downlock is still enough to handle this case by priortizing downward requests to the uplock.
...
In a usual cache-coherence protocol, when a cache object gets a response message from another object, it must have sent a corresponding request to that object.
...
As expected, in \hemiola{} we do not need to manage transient states explicitly at all; the \slstinline{rsud} rule template (standing for response-up-down) just requires additional annotation \slstinline{:holding rqM} to declare that the rule handles a response message where the ID of the original request message is \slstinline{rqM}.
...
Lastly, \autoref{fig-overview-5} deals with the case where a child finally gets a response from its parent to update its status to M, like the rule $\rdcircf{7}$.
The \slstinline{rsdd} rule template (standing for response-down-down) is very similar to the \slstinline{rsud} template but has a notable implicit difference that it releases the uplock, not the downlock in \slstinline{rsud}.
