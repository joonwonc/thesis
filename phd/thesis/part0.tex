...
\autoref{fig-motive-3} presents further rule-execution cases right after \autoref{fig-motive-2}.
After $\rdcircf{4}$ responds with \rdrecmsgsm{d}{rsI}, $P$ can take it to respond back to $C_1$.
$\rdcircf{5}$ exactly presents this step, taking \rdrecmsgsm{d}{rsI} and responding back to $C_1$ with \rdrecmsgsm{g}{rsM}.
...
Since the transient state acts like a lock, this status change can be also regarded as a lock release, so that now $P$ can handle some other requests.
...
Looking at the message channel $5$, the one for parent-to-child messages (from $P$ to $C_1$), there are two messages, \rdrecmsgsm{g}{rsM} and \blrecmsgsm{h}{rqI}, that are waiting for $C_1$ to handle them.
...
Therefore, it is crucial to have a single parent-to-child channel so that \rdrecmsgsm{g}{rsM} blocks \blrecmsgsm{h}{rqI} to be handled first.
