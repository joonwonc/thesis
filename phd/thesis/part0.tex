...
While the same concern exists in theorem proving (by requiring additional proof effort to deal with interleavings), it also has employed similar techniques.
...
The \slstinline{rsdd} rule template (standing for response-down-down) is very similar to the \slstinline{rsud} template but has a difference that it automatically releases the uplock that was previously acquired when sending the \slstinline{rqM} request.
