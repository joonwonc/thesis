...
Such a statement has become so hackneyed nowadays that it does not draw any attention, but dealing with complex hardware is still a big problem.
...
For instance, in a high level, processing units and a memory subsystem run concurrently; \eg{} a processor core may execute other instructions after making a memory request while the memory handles it.
Looking at each component, a processor core may be pipelined, which is very common in these days, implying that (computer-architecture) instructions are executed concurrently as well.
...
Concurrency does not mean complete parallelism; in order for correctness, some orders among execution units should be maintained, and managing this constraint is indeed a great challenge.
...
All the code and proofs of the framework and the case studies are available as open source in the following link:
\begin{center}
...
After that, we explore some design space of cache-coherence protocols for better understanding of the case-study protocols that we will provide in \autoref{sec-case-study}.
...
In this section, we explore major design space of conventional cache-coherence protocols.
...
A directory protocol, on the other hand, is known to be scalable and matches well hierarchical protocols, but design and verification are relatively more difficult than the snooping protocol.
...
On the other hand, in a write-back protocol, a cache does not requests to the main memory to send the up-to-date value when writing to a line.
The value is eventually written back to the main memory when the line is evicted from the cache side.
Write-back protocols are harder to implement than write-through ones, since they should provide a way to find the up-to-date value through all the caches, \eg{} by managing directories that point the cache that has the value.
...
Inclusive caches have a benefit that certain requests from the parent can be responded immediately just by searching a directory status.
For example, when a cache gets an invalidation request from the parent and the line entry does not exist in the directory, it does not need to forward the request to the children for the complete invalidation, since by the inclusion policy absense of a directory status already implies that the line does not exists in any of child caches.
...
In order to resolve the addressed issues, a number of practical cache-coherence protocols use noninclusive value caches and inclusive directories, called NCID~\cite{Zhao:2010}, \eg{} Intel Skylake-X processors are known to use the NCID structure~\cite{intel-non-inclusive,Yan:2019}.
...
Although not every combination of the policies is covered by the case studies, we would like to claim that our proposed framework \hemiola{} is general enough to design and verify cache-coherence protocols with any policies mentioned above, since the domain-specific language and verification methodologies provided by the framework are not coupled to any specific protocol policies.
