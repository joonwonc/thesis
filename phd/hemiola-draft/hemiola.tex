...
\todo{Avoid repeatedly using ``network channels.''}
...
As explained above case-by-case, transient states and network channels are the ones that block \blcircf{2} to be further proceeded.

In this sense, we would like to find a minimal conditions that ensures safe interleavings, extracted and abstracted from the transient states and network channels.
...
In later sections, we will first introduce an appropriate class of semantic models for protocols (\autoref{sec-trs}) and a formal notion of safe interleavings (called serializability) within it (\autoref{sec-sz-in-hemiola}).
We will also explain how the serializability ease the burden of verification in \autoref{sec-sz-in-hemiola} and demonstrate its meaningful use cases in verifying our case-study protocols (\autoref{sec-case-study}).
      ...
      \begin{math}
        \begin{array}{c}
          ...
          \inference[StepIns:]{\listof{m} \neq \listnil
            & \listof{\idxOf{m}} \subseteq \hsysRqA{S}}{\semstep{S}
            {\hst{\listof{o}}{M}}
            {\lblIns{\listof{m}}}
            {\hst{\listof{o}}{\enqMsgs{M}{\listof{m}}}}} \\
          \mbox{}\vspace{-5pt} \\ %% padding
          \inference[StepOuts:]{\listof{m} \neq \listnil
            & \listof{\idxOf{m}} \subseteq \hsysRsA{S}}{\semstep{S}
            {\hst{\listof{o}}{M}}
            {\lblOuts{\listof{m}}}
...
\todo{Really need to think about the notations for ``messages'' and the pair of message and its channel index, mostly used in the semantics.}
...
\begin{definition}[Trace Refinement]
  A system $I$ (``implementation'') is said to trace-refine another system $S$ (``specification''), written as $\refines{I}{S}$, iff every trace of $I$ is also a trace of $S$:
  \begin{displaymath}
...
\begin{definition}[Simulation]
  We call $(\sim): \hsyst{} \to \hsyst{} \to \propt{}$ a simulation between the systems $I$ and $S$ iff a step in $S$ exists for every step in $I$, which generates the same external label and preserves the relation:
  \begin{displaymath}
    \forall s_0, s_1, l.\; \semstep{I}{s_0}{l}{s_1} \to \forall t_0.\; s_0 \sim t_0 \to \exists t_1.\; \semstep{S}{t_0}{l}{t_1} \wedge s_1 \sim t_1.
  \end{displaymath}
\end{definition}
\begin{theorem}[Simulation implies trace refinement]
  For given systems $I$ and $S$, if there is a simulation $(\sim)$, then \refines{I}{S}.
\end{theorem}
...
In order to define serializability formally, we first provide basic definitions of atomic histories and transactions, in \autoref{fig-trs-semantics}.
A history $h$ is \emph{atomic} iff it satisfies the predicate $(\atomic{\listof{\amsgi{m}}}{h}{\listof{\amsge{m}}})$ with \emph{initial messages} $\listof{\amsgi{m}}$ and \emph{live messages} $\listof{\amsge{m}}$.
...
\todo{1) Provide more intuitions on atomic invariants, using \autoref{fig-ex-sz}}
\todo{2) Maybe it's better to introduce the notion of predicate messages here?}
\todo{3) Need to introduce the very basic notion of invariants}
\todo{4) Atomic invariants: can state and prove invariants without any interference by the other transactions}
\todo{5) Draw a diagram; what would be the predicate message if not over an atomic history?}

We call $\mathcal{I}: \hsyst{} \to \propt{}$ an \emph{invariant} over a system $S$ if $\mathcal{I}$ holds for all reachable states, \ie{} $\forall s.\; \semrch{S}{s} \to \mathcal{I}\ s$.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    % Between P and C_2
    \pic at (0, 0) {skeleton-midx-pc2};
    \node[label={[label distance=-9pt,mediumtealblue]below right:{\small $\textsf{rqI}\{\top\}$}},color=mediumtealblue] at (-0.6, -0.7) {$\bullet$};
    \node[label={[label distance=-6pt,mediumtealblue]left:{\small $\textsf{rsI}\{C_1.\textrm{st} = I\}$}},color=mediumtealblue] at (-0.8, -0.7) {$\bullet$};

    % Curves
    \draw [->,color=mediumtealblue] (-0.5, -1.2) to[out=-130,in=-120,distance=1.4cm] node[below] {\blcircf{$8$}} (-1.8, -1.0);
    \draw [->,color=mediumtealblue] (-1.6, -0.4) to[out=65,in=110,distance=1.3cm] node[above] {\blcircf{$9$}} (1.4, -0.4);
    \draw [->,color=mediumtealblue] (1.7, -1.0) to[out=-45,in=90] node[right] {\blcircf{$10$}} (2.4, -2.1);
  \end{tikzpicture}
  \caption{Predicate messages over an atomic history}
  \vspace{-5pt}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    % Curves
    \draw [->,color=mordantred19] (-0.5, -1.15) to[out=225,in=90] node[below right] {\rdcircf{$7$}\,{\small\color{mordantred19} ($C_1.\textrm{st} := M$)}} (-1.1, -1.85);
  \end{tikzpicture}
  \caption{A predicate message gets broken when interleaved}
  \vspace{-5pt}
...
An atomic invariant takes initial messages, a prestate, a poststate, and live messages from an atomic history.
Since it takes initial messages, we can specify which transaction we would like to deal with, via conditions on those messages.
