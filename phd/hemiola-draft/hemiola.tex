...
\newcommand*\rdcircf[1]{
  \adjustbox{raise=.2ex, trim=2.3pt 0pt 0pt 0pt, margin=-1pt}{
    \tikz[baseline=(char.base)]{
...
\newcommand{\rdcircfl}[2]{\rdcircf{#1}{\footnotesize #2}}
\newcommand{\blcircfl}[2]{\blcircf{#1}{\footnotesize #2}}
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \begin{tabular}{c}
      \begin{tikzpicture}
        ...
        % Curves
        \draw [->,color=myred] (-2.1, -1.8) to[out=90,in=-90] node[left] {\rdcircfl{1}{rquu}} (-1.6, -0.9);
        \draw [->,color=myred] (-1.6, -0.5) to[out=70,in=110,distance=1.4cm] node[left=15pt] {\rdcircfl{3}{rqud}} (1.4, -0.4);
        \draw [->,color=myred] (1.5, -0.9) to[out=-50,in=-50,distance=1.5cm] node[below] {\rdcircfl{4}{immu}} (0.5, -1.1);
        ...
        \node at (1.7, 0.3) {\rdcircfl{5}{rsud}};
        \draw [->,color=myred] (-0.6, -1.1) to[out=225,in=90] node[below right] {\rdcircfl{7}{rsdd}} (-1.1, -1.8);
      \end{tikzpicture}\\
      ...
      \begin{tikzpicture}
        ...
        % Curves
        \draw [->,color=myblue] (1.1, -1.8) to[out=90,in=-45] node[below left=-3pt] {\blcircfl{2}{rquu}} (0.6, -1.1);
        ...
        \node at (1.7, 0.3) {\blcircfl{6}{rqud}};
        \draw [->,color=myblue] (-0.5, -1.1) to[out=-135,in=-135,distance=1.8cm] node[below] {\blcircfl{8}{immu}} (-1.3, -0.7);
        \draw [->,color=myblue] (-1.1, -0.3) to[out=60,in=120,distance=1.3cm] node[left=17pt] {\blcircfl{9}{rsud}} (1.4, -0.5);
        \draw [->,color=myblue]  (1.6, -0.95) to[out=-90,in=90] node[above right=-2pt] {\blcircfl{10}{rsdd}} (2.1, -1.8);
      \end{tikzpicture}
    \end{tabular}
    ...
    \begin{tabular}{c}
      \multicolumn{1}{l}{
        \begin{tikzpicture}
          ...
          \draw [>=stealth,->,line width=1pt] (0, 0) to (5.5, 0);
        \end{tikzpicture}}\\
      \begin{tabular}{c|ccccccccccc}
        ...
        \hline
      \end{tabular}\\
      $\downarrow$ (commute \blcircf{6} and \rdcircf{7})\\
      \begin{tabular}{c|ccccccccccc}
        \hline
        $C_1$ & & \rdcircf{1} & & & & & \rdcircf{7} & & \blcircf{8} & & \\
        \hline
        ...
        \hline
      \end{tabular}\\
      $\downarrow^\ast$ (commute \blcircf{2} and \rdcircf{3}\rdcircf{4}\rdcircf{5}\rdcircf{7})\\
      \begin{tabular}{c|ccccccccccc}
        \hline
        $C_1$ & & \rdcircf{1} & & & & \rdcircf{7} & & & \blcircf{8} & & \\
        \hline
  ...
  \begin{subfigure}[b]{0.18\textwidth}
    ...
    \begin{tabular}{c}
      \begin{tikzpicture}
        ...
        % Curves
        \draw [->,color=myblue] (1, 0.8) to[out=-135,in=90] node[below right] {\blcircfl{6}{rqud}} (0.3, -0.2);
        \draw [->,color=myred] (0.3, -1.0) to[out=-90,in=90] node[right] {\rdcircfl{7}{rsdd}} (0.3, -1.7);
      \end{tikzpicture}\\
      \begin{tikzpicture}
        \node at (0, 0.5) {$P$};
        ...
        \node[label={[label distance=-9pt,myred]below left:{\small {\sf rsI}}},color=myred] at (0, -0.8) {$\bullet$};
        \node[label={[label distance=-4pt,myred]right:{\small {\sf rqI}}},color=myred] at (0.2, -0.8) {$\bullet$};
        \node at (0, -1.5) {$C_2$};

        % Curves
        \draw [->,color=myred] (1.2, 0.4) to[out=-135,in=90] node[below right] {\rdcircfl{3}{rqud}} (0.5, -0.6);
      \end{tikzpicture}\\
    \end{tabular}
    \caption{Commutations}
...
We call two adjacent state transitions \emph{commutable} if the state transitions with the opposite order reach to the same state:
$\semstep{S}{s_0}{l_0}{s_1} \to \semstep{S}{s_1}{l_1}{s_2} \to \exists s_1'.\; \semstep{S}{s_0}{l_1}{s_1'} \to \semstep{S}{s_1'}{l_0}{s_2}$.
Our high-level intuition is that for a given interleaved history we can perform a finite number of reductions to get a sequential history that reaches the same state.
...
We will elaborate more on this intuition with \autoref{fig-ex-sz}, two interleaving transactions started from $C_1$ and $C_2$, respectively, both pursuing the M status from the simple MSI protocol presented in \autoref{sec-example}.
\autoref{fig-ex-sz-trs} presents the two transactions (in {\color{myred} red} and {\color{myblue} blue}).
We use an arrow ({\color{myred} $\to$}) and \rdcircfl{1}{rquu} to label a state transition by a rule defined with a certain rule template, \eg{} \rdcircfl{1}{rquu} is a rule in $C_1$, taking an input message \idmsf{1}{rqWr} and making an output \idmsf{3}{rqM}.
...
The biggest contribution of the \hemiola{} framework includes the precise formalization of aforementioned intuitions -- merging atomic fragments, commutativity among the rule templates, etc. -- and the serializability proof with the topology/network and rule conditions.
...
\newcommand{\ontree}[2]{\ensuremath{\textsf{OnTree}\ #1\ #2}}
\newcommand{\goodrules}[2]{\ensuremath{\textsf{GoodRules}\ #1\ #2}}
...
\end{theorem}
While we only provide the intuition why this theorem holds in this section, interested readers are referred to \autoref{sec-appx-sz-guarantee}, which presents the required formalization and the proof of this theorem.
