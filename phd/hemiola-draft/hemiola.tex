...
All the invariants mentioned in the mergeability proof can be proven by inducting on state-transition steps.
...
\begin{proof}
  ...
  By induction on state-transition steps, we can prove that the live messages of an atomic history belong to one of the following three categories: 1) an upward request, 2) a downward response, or 3) multiple downward requests and upward responses.
  ...
  An important fact here is that since the downward response (say, to the object $O$) resides in the system after $h_1$, no history in \listof{h} performs the state transitions both in \subtree{O} and \subtreec{O}.
  Therefore, if $h \in \listof{h}$ covers the objects in \subtree{O}, it is left-pushable since $h_1$ covers \subtreec{O}.
  On the other hand, if $h$ covers \subtreec{O}, it is right-pushable.

  3) \emph{A downward request}:
  %% the invariants similar to the downward-response case are required to prove this case.
  %% If an atomic history $h$ has a downward request to the object $O$ as the initial message (in the live messages), the object state transitions by $h$ are constrained to

  4) \emph{Upward responses to the parent}:
