...
\todo{Really need to think about the notations for ``messages'' and the pair of message and its channel index, mostly used in the semantics. Also need to say implicitly every pair in the inference rules satisfies nodup(index).}
...
\begin{definition}[Atomic Invariants]
  $\mathcal{I}_a: \listtof{\hidmt{}} \to \hsyst{} \to \propt{}$ is called an \emph{atomic invariant} iff $(\mathcal{I}_a\ \listof{m_o}\; s_1)$ holds for any atomic history $h$ with $\semsteps{S}{s_0}{h}{s_1}$ and $\atomic{\listof{m_i}}{h}{\listof{m_o}}$.
