...
%% From the reviewer E:
%% 1) How do Hemiola's transactions relate to traditional coherence protocol transactions? Are they intended to be the same?
%% > (Atomic history in Hemiola) ~ (Transaction). May need to change some names?
%% 2) Your notion of "locks" appears to correspond to the notion of protocol stalls in traditional coherence terminology.
%%    Is this correct?
%% > This is correct. Maybe it's better to mention it, when talking about "locking mechanisms".
...
%% \todo{It looks better not to write the introduction like our approach is better than state-of-the-art model-checking techniques used in CCP. Let's rather say ``model checking has produced good techniques'' + ``theorem proving still suffers from dealing with interleavings'' + ``the benefit of theorem proving is that we can state and prove general theorems and reuse them repeatedly'' + ``so what is the general theorem for CCP?''}
%% \todo{This phrase attracts me a lot: design the most general form of noninterference, discover the abstract conditions that are not coupled with any specific CCPs, and use it repeatedly for various case studies.}
...
Research on model checking has produced sound techniques to overcome such state-space-explosion.
...
Modularity has been harnessed as well, e.g. with assume-guarantee reasoning~\cite{McMillan:2016}.
...
\hemiola{} exposes a domain-specific language of protocols, such that any expressible protocol is guaranteed to exhibit ``general'' noninterference, which will be called \emph{serializability} and formalized later in the paper.
...
\item Using the protocol language, we provide the complete correctness proofs of hierarchical MSI and MESI cache-coherence protocols (\autoref{sec-case-study}), for inclusive and noninclusive versions of the former and a noninclusive version of the latter. Ours are the first complete mechanized proofs for various hierarchical noninclusive cache-coherence protocols, and as a bonus they share a large segment of reusable proofs (including with inclusive variants).
\end{itemize}
...
Regarding the sequence of rule executions (in red) $[\rdcircf{1}; \rdcircf{3}; \rdcircf{4}]$ as an execution flow -- we will call it a \emph{transaction} in later sections -- to handle the original external request \rdrecmsgsm{a}{rqWr}, we see that no further executions could happen after \blcircf{2}, which is for the other request \blrecmsgsm{e}{rqWr}.
...
This mechanism to ensure safe interleavings is called \emph{noninterference}, which ensures that no other state transitions affect state relevant to a transaction while it is still running.
In model-checking cache-coherence protocols, many effective noninterference lemmas have been developed and used to avoid state explosion by interleaving transactions~\cite{McMillan:1999,McMillan:2001,Chou:2004,flow:Talupur:2008,flow:OLeary:2009,flow:Sethi:2014}.
...
We will also explain how this notion of serializability eases the burden of verification (\autoref{sec-pred-msg}) and show how it helps in verifying our case-study protocols (\autoref{sec-case-study}).
...
A \emph{message} $m$ consists of a Boolean message type (request or response), a message ID (effectively from an enumeration of message kinds), and a value.
We use \emph{value} to refer to the set of legal contents of memory addresses.
...
A rule $r$ is a struct composed of its rule index (unique within an object), a precondition (\ruleprec{}), and a transition function (\ruletrs{}).
...
It is necessary to distinguish between internal and external channels, in order to define external behaviors of the system, i.e. the interface of the cache-coherence protocol with processor cores.
...
A system may accept input messages from the external world; [StepIns] describes this case, where the external input messages (\listof{im}) should not be empty, and channels of the messages are valid; an external inputs label $(\lblIns{\listof{im}})$ is generated in this case.
...
[StepsNil] serves the case where no state transitions happen.
...
\todo{Refactor the paper so that we add new labels at the end!}
...
\todo{Here and earlier, consider collapsing the distinction between $\boolt{}$ and $\propt{}$, to avoid confusing readers who don't speak type theory.}
\begin{definition}[Simulation]
  We call $(\sim): \hstt{} \times \hstt{} \to \propt{}$ a simulation between the systems $I$ and $S$ iff a step in $S$ exists for every step in $I$, which generates the same external label and preserves the relation:
  \begin{displaymath}
...
\end{theorem}
\begin{proof}
  By induction on state-transition steps.
\end{proof}
...
These definitions are motivated by proof of cache-coherence protocols, and we postpone discussion of that specific application for the next section.
...
\end{displaymath}
We will write just \predmsg{im}{P} when the system $S$ is clear from context, also often using an even-shorter version \predmsg{\msgid{}}{P} (considering only those messages with a given ID) when it is not ambiguous.
...
Recalling the example protocol from \autoref{sec-example}, when the system has a message of a kind \blmsg{rsM}, we expect the parent and the other child (who is not the requestor) to have I status (like {\color{myblue} $\{C_1.\textrm{st} = I \wedge P.\textrm{st} = I\}$} in the figure).
...
\begin{definition}[Atomic Invariants]
  $\mathcal{I}_a: \listtof{\hidmt{}} \times \hstt{} \to \propt{}$ is called an \emph{atomic invariant} iff $(\mathcal{I}_a\ \listof{im_o}\; s_1)$ holds for any atomic history $h$ with $\semsteps{S}{s_0}{h}{s_1}$ and $\atomic{\listof{im_i}}{h}{\listof{im_o}}$.
...
The other three cases ([TrsSilent], [TrsIns], and [TrsOuts]) do not need any further information, since each case induces just a single step.
...
In this section, we would like to provide \emph{minimal and abstract conditions} to prove serializability, in order not to prove it directly for each protocol.
The conditions have already been mentioned with the motivating example in \autoref{sec-example} -- network topology and locking mechanisms, extracted from transient states of practical cache-coherence protocols.
...
\hemiola{} provides a set of rule templates for describing protocols in a way that satisfies the serializability conditions by-construction.
\autoref{fig-rule-templates} presents the nine rule templates.
...
It means that the rule template is for an object $O$, requires input messages ($\circ$) with the message types determined by the label, has a precondition $P$, performs a state transition $Q$, and generates output messages ($\bullet$).
...
\begin{figure*}[t]
  ...
  \begin{subfigure}[b]{0.40\textwidth}
    ...
    \begin{tabular}{c}
      ...
      \hspace{10pt} by commuting \blcircf{6} and \rdcircf{7})\\
      \vspace{-10pt}\\
      \begin{tabular}{c|ccccccccccc}
        \hline
        $C_1$ & & \rdcircf{1} & & & & & \rdcircf{7} & & \blcircf{8} & & \\
...
We say that two adjacent state transitions \emph{commute} if the state transitions with the opposite order reach the same state:
\begin{displaymath}
  \semstep{S}{s_0}{l_0}{s_1} \land \semstep{S}{s_1}{l_1}{s_2} \to \exists s_1'.\; \semstep{S}{s_0}{l_1}{s_1'} \wedge \semstep{S}{s_1'}{l_0}{s_2}.
\end{displaymath}
...
We use an arrow ({\color{myred} $\to$}) and a label (\rdcircfl{1}{rquu}) to denote a state transition by a rule defined with a certain rule template, \eg{} \rdcircfl{1}{rquu} is a rule in $C_1$ using the ``rquu'' rule template, taking an input message \idmsf{1}{rqWr} and outputting \idmsf{3}{rqM}.
...
We will say that $[\blcircf{6}]$ is \emph{pushed} to the left (or right) when it commutes with $[\rdcircf{3}; \rdcircf{4}; \rdcircf{5}]$ (or $[\rdcircf{7}]$).
...
In the first merge case, $[\blcircf{6}]$ can be pushed to the right (\ie{} \blcircf{6} and \rdcircf{7} commute).
...
\autoref{fig-ex-sz-comm} (top) depicts this case; we see \blcircf{6} and \rdcircf{7} still commute, since \rdcircfl{7}{rsdd} should take a ``response'' (\rdmsgsm{rsM}) while \blcircfl{6}{rqud} outputs a ``request'' (\blmsgsm{rqI}).
...
These state transitions still commute, as shown in \autoref{fig-ex-sz-comm} (bottom), since \blcircf{2} cannot make any object state transition, according to the definition of the ``rquu'' rule template.
...
\todo{Describe inclusive protocol.}
...
\tikzset{
  pcc1/.pic = {
    ...
    \draw [>->] (0.2, -0.3) -- (0.6, -0.8);
    % message
    \node[color=myblue,label={[label distance=-10pt]above right:{\blmsgsm{rqS}}}] at (0.8, -0.55) {$\bullet$};
  },
  pcc2/.pic = {
    \node at (0, 0) {\color{myred} $P(\stS{}, v, \dir{\stS{}}{1,2}, \top)$};
    \node at (-1.1, -1.1) {\color{myred} $C_1(\stS{}, v)$};
    \node at (1.1, -1.1) {$C_2(\stI{}, \cdot)$};
    % to the parent of P
    \node at (0, 1) {$\vdots$};
    \draw [>->] (-0.2, 0.3) -- (-0.2, 0.6);
    ...
    \draw [>->] (0.2, -0.3) -- (0.6, -0.8);
    % message
    \node[color=myred,label={[shift={(0.3,-0.25)}]left:{\rdmsgsm{rsS}{\color{myred}$(v)$}}}] at (0.4, -0.55) {$\bullet$};
  }
}
\begin{figure}[h]
  \centering
...
In order to make the execution of these pipeline-stage rules correct, as per standard hardware pipelines, the pipeline includes stall logic to make concurrent partial executions for different line addresses correct.
...
Each protocol used just the right amount of BRAMs for data caches and information caches that contain status, directory bits, etc.
\todo{Explain what ``just the right amount'' means.}
...
Optimization and verification of the cache-controller design are nontrivial; the pipeline requires correct stall logic, which is as sophisticated as the logic in pipelined processors~\cite{comarch}.
While the verification of the pipeline is one of our future-work directions, we see it as orthogonal to the verification of cache-coherence protocols, which is what \hemiola{} focuses on.
...
%% Therefore, during the merge it is crucial to ensure that no other fragment between the two fragments is separated.
...
Recall that the serializability (defined in \autoref{sec-seq-sz}) does not require to preserve the behavior of a history, but the sequential history to reach to the same state.
...
\begin{definition}[Interleaved Histories]
  ...
  \end{align*}
  We also call a history $h$ interleaved (overrided as \hitlv{S}{h}) if we can find an interleaved sequence of histories $\listof{h_s}$ that satisfies $h = \listconcat{\listof{h_s}}$.
\end{definition}
...
\begin{definition}[Nonconfluence]
  ...
  \end{align*}
  where \extatomicShort{S}{h} is a shorthand for $(\extatomic{S}{\listof{\amsgi{im}}}{h}{\listof{\amsge{im}}})$ for some \listof{\amsgi{im}} and \listof{\amsge{im}}.
\end{definition}
...
\begin{definition}
  For a given system $S$, two atomic histories $h_1$ and $h_2$ (not necessarily continuous), and an atomic history $h$ between the two, $h$ is \emph{left-pushable} (\emph{right-pushable}) iff it can be pushed right before $h_1$ (after $h_2$), respectively:
  \begin{displaymath}
    \renewcommand{\arraystretch}{1.6}
    \begin{array}{l}
      \lpush{S}{h_1}{h_2}{h} \triangleq\\
      \quad \forall s_1, s_2.\; \semsteps{S}{s_1}{\listapp{h_2}{\listapp{h}{h_1}}}{s_2}
      \to \semsteps{S}{s_1}{\listapp{h_2}{\listapp{h_1}{h}}}{s_2}.\\
      \rpush{S}{h_1}{h_2}{h} \triangleq\\
      \quad \forall s_1, s_2.\; \semsteps{S}{s_1}{\listapp{h_2}{\listapp{h}{h_1}}}{s_2}
      \to \semsteps{S}{s_1}{\listapp{h}{\listapp{h_2}{h_1}}}{s_2}.\\
    \end{array}
  \end{displaymath}
\end{definition}

We could define mergeability directly using pushability, but we define and use the following relaxed definition:
\begin{definition}
...
(The full mechanized Coq proof is in the supplementary material.)
Recalling the requirements from \autoref{sec-dsl}, a protocol should be defined on a tree topology (\ontree{S}{t}), and each rule should be defined using the rule templates (\goodrules{S}{t}).
...
\begin{proof}
  ...
  Now according to what is specified by the designated rule template for making requests to children (by \goodrules{S}{t}), the parent sets a downlock and does not accept any other atomic histories to make another set of requests to the children.
...
The mergeability proof is trickier, requiring a number of invariants about specific locks and messages.
All the invariants mentioned in the proof should be provable by inducting on state-transition steps.
...
\begin{proof}
  ...
  Following the definition of mergeability, let $h_1$ and $h_2$ be externally continuous (\hextcont{S}{h_1}{h_2}), \ie{} $h_1$ is externally atomic and $h_1$ and $h_2$ are continuous.
  As already observed and used in the nonconfluence proof (\autoref{thm-ncf}), the initial messages of $h_2$ (say \ainits{h_2}) belong to the four categories: an upward request, a downward response, a downward request, or multiple upward responses (to the parent).

  In addition to this categorization, in proving mergeability, we need to categorize the live messages of $h_1$ (say \alives{h_1}) as well, which is formed as an invariant.
  By induction on state-transition steps, we can prove that the live messages of an external atomic history belong to one of the following three categories: 1) an upward request, 2) a downward response, or 3) multiple downward requests and upward responses.
  ...
  The following case analyses are by the initial messages of $h_2$.

  \vspace{5pt}
  1) \emph{An upward request}: since $\ainits{h_2} \subseteq \alives{h_1}$, \alives{h_1} should contain an upward request, and due to the categorization of \alives{h_1} it should be a single upward request as well.
  We need to prove an invariant here, claiming that if an atomic history has an upward request as live messages, it only consists of internal labels generated by the ``rquu'' rule template, \ie{} taking an upward request from a child and forwarding it to the parent.
  Now considering the history $h$ between $h_1$ and $h_2$, it is always left-pushable, since the ``rquu'' rules does not make any local state transition, and uplocks set by these rules do not allow $h$ to use any upward-request channels used by $h_1$.

  \vspace{5pt}
  2) \emph{A downward response}: by the same reasoning, \alives{h_1} should be a single downward response.
  We need two dual invariants to prove this case: if an atomic history $h$ has a downward response to the object $O$ as the initial message (the live message), the object state transitions performed by $h$ are constrained to \subtree{O} (\subtreec{O}), respectively.
  Let us call $O$ the \emph{separation point}, which will be used in the other cases as well.
  ...
  An important fact here is that since the downward response (say, to the object $O$) resides in the system after $h_1$, no history in \listof{h} can perform the state transitions both in \subtree{O} and \subtreec{O}.
  Therefore, if $h \in \listof{h}$ is constrained to the objects in \subtree{O}, it is left-pushable since $h_1$ is within \subtreec{O}.
  On the other hand, if $h$ is within \subtreec{O}, it is right-pushable.

  \vspace{5pt}
  3) \emph{A downward request}: the required invariants to prove this case is similar to the ones used in (2).
  Suppose that the request is to the object $O$.
  It would be great if the separation point is also $O$ as same as (2), but it is more nontrivial in this case.
  This is because $h_2$ has a chance to make state transitions in the outside of $O$ (\ie{} \subtreec{O}) eventually after dealing with the corresponding response from $O$.

  The separation point is what we call \emph{the root of downward requests and upward responses}, defined as the first ancestor of $O$ that has a downlock set by an upward request, say $O_r$.
  (There are two ways to make a downlock: one by an upward request and the other by a downward request.)
  Being $O_r$ as the separation point, we can prove that $h_1$ is constrained to $(\subtreec{O_r} \cup \{O_r\})$ and $h_2$ is within \subtree{O_r}.

  There is a possible overlap between $h_1$ and $h_2$ in $O_r$, which is fine since the intermediate histories cannot make any state transitions to $O_r$ due to its downlock.
  Specifically, each history $h \in \listof{h}$ can make state transitions within either $(\subtree{O_r} - \{O_r\})$ or \subtreec{O_r}.
  Similar to (2), we can then decide whether $h$ is left- or right-pushable.

  4) \emph{Upward responses to the parent}: suppose that the responses are from $\{C_i\}_{i \in I}$ (where $I$ is the set of children indices that sent a response) to the object $O$.
  The separation in this case is the most complex, but the proof concept is still the same.
  The separation point is $O_r$, and $h_1$ is within $((\subtreec{O_r} \cup \{O_r\}) \cup (\bigcup_{i \in I}\ \subtree{C_i})$ and $h_2$ is within $(\subtree{O_r} - (\bigcup_{i \in I}\ \subtree{C_i}))$.
  Each history $h \in \listof{h}$ can make state transitions within either $(\subtreec{O_r} \cup (\bigcup_{i \in I}\ \subtree{C_i}))$ (thus right-pushable) or $((\subtree{O_r} - \{O_r\}) - (\bigcup_{i \in I}\ \subtree{C_i}))$ (left-pushable).
\end{proof}
\renewcommand*{\proofname}{Proof}

Finally we combine all together to obtain the serializability in \hemiola{}:
\begin{theorem}[\hemiola{}'s serializability guarantee]
  \begin{displaymath}
    \forall S, t.\; \ontree{S}{t} \land \goodrules{S}{t} \to \hsrz{S},
  \end{displaymath}
  \vspace{-15pt}
  \label{thm-sz-guarantee-pf}
\end{theorem}
\begin{proof}
