...
\lstset{
  ...
  emphstyle={\bfseries\color{mordantred19}},
...
\newcommand*\rdrecsm[1]{
  \adjustbox{raise=.1ex, outer=8.5pt}{
...
\newcommand*\rdcircf[1]{
  \adjustbox{raise=.2ex, outer=8.5pt}{
    \tikz[baseline=(char.base)]{
      \node[shape=circle, draw, inner sep=0pt, minimum size=8pt, fill=mordantred19, text=white]
      (char) {\tiny\sffamily #1};}}
}
\newcommand*\blcircf[1]{
  \adjustbox{raise=.2ex, outer=8.5pt}{
    \tikz[baseline=(char.base)]{
      \node[shape=circle, draw, inner sep=0pt, minimum size=8pt, fill=mediumtealblue, text=white]
      (char) {\tiny\sffamily #1};}}
}
...
\begin{figure}[h]
  ...
  \begin{tabular}{c}
    \begin{tikzpicture}
      ...
      % Curves
      \draw [->,color=mordantred19] (-2.8, -2.1) to[out=90,in=-110] node[left] {\rdcircf{1}} (-2.2, -0.7);
      \draw [->,color=mordantred19] (-2.2, -0.6) to[out=70,in=120,distance=1.8cm] node[above] {\rdcircf{3}} (2.1, -0.5);
      \draw [->,color=mordantred19] (2.1, -0.6) to[out=-60,in=-60,distance=1.2cm] node[right] {\rdcircf{4}} (1.9, -1);
      \draw [->,color=mediumtealblue] (0.9, -1.9) to[out=90,in=-45] node[outer sep=-3pt, below left] {\blcircf{1}} (0.3, -1.2);
    \end{tikzpicture}\\
    %% \footnotesize
    %% \begin{tabular}{|c|c|c|c|}
    %%   \hline
    %%   & Inputs & Status change & Outputs \\
    %%   \hline
    %%   \rdcircf{1} & \rdmsg{a}{rqM} & $C_1: S \rightarrow S_2M$ & \rdmsg{b}{rqM} \\
    %%   \blcircf{1} & \blmsg{e}{rqM} & $C_2: S \rightarrow S_2M$ & \blmsg{f}{rqM} \\
    %%   \rdcircf{3} & \rdmsg{b}{rqM} & $P: S \rightarrow S  & \rdmsg{c}{rqI} \\
    %%   \rdcircf{4} & \rdmsg{c}{rqI} & & \rdmsg{d}{rsI} \\
    %% \end{tabular}
  \end{tabular}
...
\rdcircf{1} is the case where a child $C_1$ takes an external request (\rdmsgsm{a}{rqM}) to write data, but it does not have M status and thus forwards the request to the parent (\rdmsgsm{b}{rqM}).
At this moment the child changes its status to a \emph{transient state} SM to record that it just forwarded the message to get the M status.
...
As a next rule-execution step, \rdmsgsm{b}{rqM} (previously outputted by \rdcircf{1}) is handled by the parent $P$.
\rdcircf{3} shows the transition, making \emph{invalidation} requests (\rdmsgsm{c}{rqI}) to the other children ($C_2$ is the only other child in this system) to change their statuses to I, since when a child has M the others should not be able to read/write the data.
After making invalidation requests, the parent also changes its status to a transient state (sets a lock) to disallow any other requests from the children.

Now imagine a scenario where two children $C_1$ and $C_2$ requested \rdmsgsm{b}{rqM} and \blmsgsm{f}{rqM} to the parent (to obtain M) at the same time, respectively, and the parent decided to deal with the request from $C_1$ first.
Proper locking is indeed required here, since otherwise the parent will handle two {\sf\small rqM}s simultaneously, which might lead to an incoherent state -- two M statuses in the system.

\rdcircf{3} presents this case, where an invalidation request was made to $C_2$ (\rdmsgsm{c}{rqI}) and even before this, $C_2$ requested \blmsgsm{f}{rqM} to the parent.
In order to handle both requests correctly, we should carefully consider some corner cases:
\begin{itemize}[leftmargin=*]
\item Since $C_2$ requested \blmsgsm{f}{rqM}, it has a transient state SM when \rdmsgsm{c}{rqI} arrives. It should still be able to handle this invalidation request even if it requested the status change to M. We see that locking (by the transient states) should be fine-grained enough to distinguish message kinds while handling a request.
...
\section{WIP: an example of interleaving transactions and their serialization}
