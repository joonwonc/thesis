...
\todo{We need to put some intuition on noninterference and serialization here as well.}
...
As explained in \autoref{sec-sz-in-hemiola}, on top of the serializability guarantee, it is much easier to design and prove invariants that help proving the correctness of cache-coherence protocols.
That being said, it will be very tedious if a user has to prove the serializability per-protocol.
...
The conditions have been already mentioned with the motivating example in \autoref{sec-example} -- network topology and locking mechanisms extracted from transient states that have been used practically for a long time.

While conventional approaches to verifying cache-coherence protocols deal with transient states directly and induce noninterference lemmas per-state, \hemiola{} provides serializability as \emph{the most general form of noninterference, with a minimal set of abstract conditions that are not coupled with any specific cache-coherence protocols}.
