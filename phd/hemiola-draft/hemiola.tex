...
Definition liDropImm: Rule :=
  rule.imm
  :requires (fun ost _ _ =>
    ost#[status] (*$\leq$*) msiS /\ ost#[owned] = false)
...
\todo{Change it to the ordinary invalidation-request rule; explain the messages rqI and rsI.}
...
Its precondition just says that the line is possibly shared (\slstinline{ost#[status]} $\leq$ \slstinline{msiS}) but needs not be written back (\slstinline{ost#[owned] = false}).
...
We already discussed in \autoref{sec-atomic-invariants} that a number of invariants are necessarily required to prove simulation.
...
For instance, we want to claim that whenever a cache has \stM{} status, all the other caches have \stI{} status.
\todo{This is wrong.}
...
In order to deal with this subtlety, we introduce a notion called \emph{logical status} to obtain an abstract status of each cache.
...
\begin{itemize}[leftmargin=*]
\item A cache has logical status \stM{} iff it has \stM{} and there is no invalidation response to it.
\item A cache has logical status \stS{} iff either 1) it has \stS{} and there is no invalidation response to it or 2) there is a response \slstinline{rsS} to it.
\item A cache has logical status \stI{} iff either 1) it has \stI{} and there is no \slstinline{rsM} or \slstinline{rsS} to it or 2) there is an invalidation response \slstinline{rsI} to it.
\end{itemize}
Note that the logical status of a cache is \emph{not} \stM{} when there is a response \slstinline{rsM} to it, since the response could imply an ongoing invalidation process.

\paragraph{Invariants.}
Using the logical status of each cache, we can state the invariant for each status:
\begin{itemize}
...
\item \todo{Clarify more about a ``certain'' set; say it will be further explained soon.} The \emph{invalidness} invariant claims that a certain set of caches are all in logical \stI{} status. Such a set may refer to the caches inside (or outside) a subtree of the system, determined by looking at the directory status or the ownership bit. For example, if a cache $O$ has an ownership bit true, then all the caches outside $O$ have logical \stI{} status.
\end{itemize}
