...
While the notions of interleaving and serialization are already pervasive, I have found no past work presenting a minimal set of conditions that ensure transactions are always serialized, formalizing them in a reusable framework.
...
To demonstrate usability of the framework, we plan to provide complete correctness proofs of various hierarchical cache-coherence protocols as case studies, including noninclusive protocols.
...
Before proposing the \hemiola{} framework, we provide a simple yet motivating example to introduce cache-coherence protocols and typical challenges in their design and verification.
...
\subsection{Correctness proofs of hierarchical noninclusive MSI and MESI protocols}
\label{sec-casestudy}
...
The correctness proofs of the MSI and MESI protocols claim that both implementations refine to a single-line memory as a spec.
Most of the time, a refinement proof requires two steps: 1) to prove \emph{invariants} of the implementation that help relate it to the spec and 2) to define and prove a \emph{simulation} relation between an implementation and the spec that directly implies a refinement.
...
\subsubsection{Optional task: design and proof of an inclusive MSI protocol}

As motivated in \autoref{sec-intro}, we set the noninclusive protocols as case studies to demonstrate that \hemiola{} can support verification of certain cache-coherence protocols that the other approaches have hardly dealt with.
However, we claim that the proposed design methodology and proof techniques -- rule templates, serializability, logical statuses, predicate messages, etc. -- are general enough to be used for some other protocols.
In order to show the practicality of the framework, if time permits, we plan to design and prove a hierarchical \emph{inclusive} MSI protocol as an extra case study.
While the inclusive protocol requires additional rules (\eg{} ones for back invalidation) to be defined, we expect to reuse a significant amount of invariant proofs from the noninclusive protocol, since a lot of rules are shared between the two.
...
To improve space efficiency, we plan to implement NCID~\cite{Zhao:2010,Yan:2019}, which already has been used in industry.

Another prebuilt component holds a finite number of miss status holding registers (MSHRs), whose abstract interface includes registering, updating, and releasing MSHRs with respect to their types (uplock or downlock) and locking addresses.
...
\begin{figure}[h]
  ...
  \begin{tabular}{r|l}
    ...
    \hline
    \multirow{3}{*}{Oct 2020} & {\bf Complete proofs of the case-study protocols (\autoref{sec-casestudy})} \\
    & \quad Clean up all the proofs \\
    & \quad (Design and prove an inclusive MSI protocol) \\
    \hline
    ...
    & \quad (Prove compiler correctness) \\
    \hline
    \multicolumn{2}{l}{} \\
    \multicolumn{2}{l}{\normalsize\bf Thesis} \\
    \hline
...
The \hemiola{} framework has been developed over around 2 years; the domain-specific language and the main serializability proof have already been established.
...
If the cleanup takes shorter than expected, an inclusive MSI protocol will be designed and proven, in order to claim the practicality of the framework better.
The protocol compiler, on the other hand, still requires a decent amount of work.
